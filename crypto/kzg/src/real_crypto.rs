//! Real KZG cryptographic implementation using c-kzg and rust-eth-kzg.

use crate::{KzgCommitment, KzgProof, BYTES_PER_BLOB, CELLS_PER_EXT_BLOB};
use rust_eth_kzg::{CellIndex, DASContext};
use std::collections::HashMap;
use std::fmt::Debug;

pub use c_kzg::{Blob, Bytes32, Bytes48, KzgSettings};
pub use rust_eth_kzg::{Cell, CellIndex as CellID, CellRef, TrustedSetup as PeerDASTrustedSetup};
...
impl Kzg {
    ...
    pub fn compute_cells(
        &self,
        blob: KzgBlobRef<'_>,
    ) -> Result<[rust_eth_kzg::Cell; CELLS_PER_EXT_BLOB], Error> {
        self.context()
            .compute_cells(blob)
            .map_err(Error::PeerDASKZG)
    }
